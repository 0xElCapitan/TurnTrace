...
CLI:  python eval/run_eval.py            # → runs/run-0001
      python eval/run_eval.py --run-id run-0001 --out-dir runs/run-0001
"""
...
from run_match import deck_hash as compute_deck_hash, run_single, write_match  # noqa: E402
...
def run_eval(run_id: str, out_dir: Path, regime_id: str = "regime-v001",
    ...
    agent_version = random_legal_agent.AGENT_VERSION
    manifest = {
        "run_id": run_id,
        "regime_id": regime_id,
    ...
    for idx, match_id in zip(match_indices, expected_match_ids):
        ...
        inputs = {
            "agent_a_id": "random_legal", "agent_b_id": opponent_id,
    ...
    # ---- summary.csv + exactly one ledger row (delegated; AC-5, AC-7) ----
    agg = aggregate.aggregate_and_ledger(
        ...
        notes="mode=unseeded; timeout undetectable (soft gate); illegal-action gate hard (detectable=true).",
    )
    ...
    return {"run_dir": str(out_dir), "n": len(expected_match_ids),
            "stats": agg["stats"], "ledger_appended": agg["ledger_appended"],
            "claim_ceiling": agg["claim_ceiling"]}
